...
import { encodeXRsv, isValidXRsv } from "./xrsv.ts"
...
function saveValidTestCase(i: number, rows: (string | null)[][]) {
	...
	if (isValidXRsv(bytesA) === false) { throw new Error("Validation mismatch") }
	...
	//Deno.writeTextFileSync(path+".xml", xrsvToXml(rows))
	//Deno.writeTextFileSync(path+".sml", "\uFEFF"+xrsvToSml(rows))
}
...
function saveInvalidTestCase(i: number, byteArray: number[]) {
	...
	if (isValidXRsv(bytes) === true) { throw new Error("Validation mismatch") }
}
